...
class Board():
    ...
    def check_move(self, s, t, player):
        ...
        if s < 0 or s > 15 or t < 0 or t > 15: 
            valid_flag = False
        if self.state[s]== player.get_symbol() and self.state[t]=='.':
            valid_flag = False
            #print(self.state[s])
            #print(player.get_symbol())
            length = len(self.edges)
            for i in range(length):
                temp = set(self.edges[i]) & set([s,t])
                if len(temp)==2:
                    valid_flag = True
        else:
            valid_flag = False
    ...
    def form_mill(self, pos, player):
        ...
        for [i,j,k] in self.mills:
            if self.state[i]==symbol and self.state[j]==symbol and self.state[k]==symbol:
    ...
    def check_win(self, player, opponent):
        ...
        oppSymbol = opponent.get_symbol()

        for i in range(16):
            if(self.state[i]==oppSymbol):
